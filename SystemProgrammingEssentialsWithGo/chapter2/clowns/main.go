...
func main() {
	clownChannel := make(chan int, 45)
	clowns := 50
	clownsLeftBehind := make([]int, 0)
	...
	for clown := 1; clown <= clowns; clown++ {
		...
		go func(clownID int) {
			...
			default:
				...
				clownsLeftBehind = append(clownsLeftBehind, clownID)
			}
		}(clown)
	}
	wg.Wait()
	fmt.Printf("There are the clowns that did not make it: %d", clownsLeftBehind)
}
